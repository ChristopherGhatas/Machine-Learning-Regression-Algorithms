...
#add 1 column at the end of your dataset fro backward elimination.
...
# removing column 2 since it's p>sl (sl=0.05)
x_opt = x[:, [0, 1, 3, 4, 5]]
regressor_OLS = sm.OLS(endog = y , exog = x_opt).fit()
regressor_OLS.summary()

# removing column 1 since it's p>sl (sl=0.05)
x_opt = x[:, [0, 3, 4, 5]]
regressor_OLS = sm.OLS(endog = y , exog = x_opt).fit()
regressor_OLS.summary()

# removing column 4 since it's p>sl (sl=0.05)
x_opt = x[:, [0, 3, 5]]
regressor_OLS = sm.OLS(endog = y , exog = x_opt).fit()
regressor_OLS.summary()

# removing column 5 since it's p>sl (sl=0.05)
x_opt = x[:, [0, 3]]
regressor_OLS = sm.OLS(endog = y , exog = x_opt).fit()
regressor_OLS.summary()
